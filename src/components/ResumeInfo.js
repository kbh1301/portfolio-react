export const resume = [
    {
        id: 0,
        title: 'Employment History',
        data: [
            {
                id: 0,
                position: 'Guidewire Configuration Developer',
                place: 'Farm Bureau TN',
                date: 'Feb 2020 - Present',
                overview: 'Working with the configuration team to develop Guidewire Insurance Suite in accordance to Farm Bureau’s specific needs.',
                descs: [
                    {
                        id: 0,
                        desc: 'Developing and pushing Java-based Gosu code changes via Git',
                    },
                    {
                        id: 1,
                        desc: 'Unit testing and logging changes in accordance to company standards',
                    },
                    {
                        id: 2,
                        desc: 'Coordinating sprint work using Rally and SharePoint',
                    }
                ]
            },
            {
                id: 1,
                position: 'IT Department Intern',
                place: 'EnableComp',
                date: 'Jan 2019 - May 2019',
                overview: 'Worked with team members of various information technology specializations such as networking, cybersecurity, computer programming, and help desk.',
                descs: [
                    {
                        id: 0,
                        desc: 'Assisted in setting up hardware',
                    },
                    {
                        id: 1,
                        desc: 'Provided technical support via ticketing system',
                    },
                    {
                        id: 2,
                        desc: 'Shadowed and assisted in software development',
                    },
                    {
                        id: 3,
                        desc: 'Received training in debugging, troubleshooting, and testing'
                    }
                ]
            },
            {
                id: 2,
                position: 'Rural Carrier Associate',
                place: 'United States Postal Service',
                date: 'July 2018 - Dec 2018',
                overview: 'Performed delivery and collection services for all rural classified routes in the Tullahoma area.',
                descs: [
                    {
                        id: 0,
                        desc: 'Prepared mail and packages for delivery',
                    },
                    {
                        id: 1,
                        desc: 'Delivered on foot and by vehicle',
                    },
                    {
                        id: 2,
                        desc: 'Assisted other team members to cover all delivery routes',
                    }
                ]
            },
            {
                id: 3,
                position: 'Store Associate',
                place: 'K&S Awards',
                date: 'February 2018 - July 2018',
                overview: 'Performed a wide variety of duties necessary to run a small awards business with store owners. Duties included sales, friendly service with clients, maintaining Excel database, building awards, and more.',
                descs: [
                    {
                        id: 0,
                        desc: 'Managed store independently when necessary',
                    },
                    {
                        id: 1,
                        desc: 'Designed and built various awards to specifications',
                    },
                    {
                        id: 2,
                        desc: 'Worked with various hardware and software programs',
                    }
                ]
            }
        ]
    },
    {
        id: 1,
        title: 'Education',
        data: [
            {
                id: 0,
                position: 'AAS in Computer Information Technology, Programming Concentration',
                place: 'Columbia State Community College',
                date: 'May 2019 - August 2020',
                overview: '',
                descs: [
                    {
                        id: 0,
                        desc: 'Focus on Java and C# development',
                    },
                    {
                        id: 1,
                        desc: 'Graduated Magna Cum Laude',
                    },
                    {
                        id: 2,
                        desc: 'Student representative of 2020 CITC advisory board',
                    }
                ]
            },
            {
                id: 1,
                position: 'Interested in pursuing Bachelor of Computer Science',
                place: '',
                date: 'Future Goal',
                overview: '',
                descs: []
            },
            {
                id: 2,
                position: 'Udemy Certification',
                place: 'The Complete Web Developer in 2021: Zero to Mastery',
                date: 'In Progress',
                overview: '',
                descs: []
            },
            {
                id: 3,
                position: 'High School Diploma',
                place: 'Franklin Christian Academy',
                date: 'May 2013',
                overview: '',
                descs: []
            }
        ]
    },
    {
        id: 2,
        title: 'Related Skills',
        data: [
            {
                id: 0,
                position: 'Languages',
                place: '',
                date: '',
                overview: '',
                descs: [
                    {
                        id: 0,
                        desc: 'Java',
                    },
                    {
                        id: 1,
                        desc: 'SQL',
                    },
                    {
                        id: 2,
                        desc: 'HTML, CSS, JavaScript',
                    },
                    {
                        id: 3,
                        desc: 'C#, C++',
                    },
                    {
                        id: 4,
                        desc: 'Gosu',
                    }
                ]
            },
            {
                id: 1,
                position: 'Tools',
                place: '',
                date: '',
                overview: '',
                descs: [
                    {
                        id: 0,
                        desc: 'Visual Studio',
                    },
                    {
                        id: 1,
                        desc: 'VS Code',
                    },
                    {
                        id: 2,
                        desc: 'GitHub',
                    },
                    {
                        id: 3,
                        desc: 'Bitbucket',
                    },
                    {
                        id: 4,
                        desc: 'Rally',
                    },
                    {
                        id: 5,
                        desc: 'Guidewire Studio',
                    },
                    {
                        id: 6,
                        desc: 'Sublime Text Editor',
                    },
                    {
                        id: 7,
                        desc: 'JGrasp',
                    },
                    {
                        id: 8,
                        desc: 'React',
                    },
                    {
                        id: 9,
                        desc: 'npm',
                    },
                    {
                        id: 10,
                        desc: 'Sharepoint',
                    }
                ]
            },
            {
                id: 2,
                position: 'Concepts',
                place: '',
                date: '',
                overview: '',
                descs: [
                    {
                        id: 0,
                        desc: 'Agile Methodology',
                    },
                    {
                        id: 1,
                        desc: 'Debugging',
                    },
                    {
                        id: 2,
                        desc: 'Version Control',
                    }
                ]
            }
        ]
    }
];
  